...
function makeNodeState(tile: Tile, keyStr: string, entry): Node {
...
export function scheduleRefetch() {
    ...
    refetchTimer = setTimeout(() => {
        ...
        if (
            Math.hypot(wx - fetchCenter.x, wy - fetchCenter.y) > REFETCH_THRESHOLD
        ) {
            refreshTiles(wx, wy);
        }
    }, 500);
...
export async function refreshTiles(cx: number, cy: number) {
    ...
    try {
        ...
        let tileInfos;
        try {
            tileInfos = await fetch(`/api/find?x=${cx}&y=${cy}&r=${FETCH_RADIUS}`)
            .then((r) => r.json() as Promise<APITileInfo[]>);
        } catch (err) {
