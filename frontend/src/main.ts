...
import {cameraSet, target, sph} from "./renderer.ts";
import {setHeightLimits} from "./colours.ts";
import {initEventHandlers} from "./input.ts";
import {setColourMode, colourMode} from "./colours.ts";
...
function loadCameraFromURL() {
    ...
    if (s?.length === 3) {sph.theta = s[0]; sph.phi = s[1]; sph.r = s[2];};
...
async function init() {
    ...
    cameraSet(center_x - 5000, center_y - 5000, bounds.zmin, center_x + 5000, center_y + 5000, bounds.zmax);
