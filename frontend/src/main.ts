...
import { cameraInit, sph, target } from "./renderer.ts";
...
import { APIOverview } from "./types.ts";
...
function loadCameraFromURL() {
    ...
    if (toggles?.length === 3) {
        (document.getElementById("p2r-check")! as HTMLInputElement).checked = toggles[0] === 1;
        (document.getElementById("pitfree-check")! as HTMLInputElement).checked = toggles[1] === 1;
        (document.getElementById("cameras-check")! as HTMLInputElement).checked = toggles[2] === 1;
    }
    if (params.has("cm")) {
        setColourMode(params.get("cm")!);
        document.querySelectorAll(".cm-btn").forEach(b => {
            b.classList.toggle("active", (b as HTMLElement).dataset.mode === colourMode);
        });
    }
}
...
async function init() {
    ...
    const bounds = await fetch(`/api/overview`).then((r) => r.json() as Promise<APIOverview>);
