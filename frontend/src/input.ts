...
import {renderer, cameraReset, target, sph, resizeEvent, render} from "./renderer.ts";
import {scheduleLODUpdate, scheduleRefetch, tileStates} from "./tiles.ts";
import {colourMode, setColourMode, recolorAll} from "./colours.ts";
...
function saveCameraToURL() {
    ...
    const hash = `s=${fmt(s.theta)},${fmt(s.phi)},${fmt(s.r)}&t=${fmt(t.x)},${fmt(t.y)},${fmt(t.z)}&o=${toggles[0]},${toggles[1]},${toggles[2]}&cm=${colourMode}`;
...
export function initEventHandlers() {
    ...
    document.querySelectorAll('.cm-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById('height-legend').style.display = colourMode === 'height'          ? '' : 'none';
            document.getElementById('cls-legend').style.display    = colourMode === 'classification'  ? '' : 'none';
            document.getElementById('lc-legend').style.display     = colourMode === 'landcover'       ? '' : 'none';
            recolorAll(tileStates);
            render(tileStates);
            saveCameraToURL();
        });
    });
    ...
    window.addEventListener('resize', () => {resizeEvent(tileStates)});
}